...
router.get('/games', (req, res) => {
  try {
    ...
    const list: GameListItem[] = games.map((g) => ({
      id: g.id,
      level: g.level,
      status: g.status,
      steps: g.steps,
    ...
    const response: ApiResponse<GameListItem[]> = {
      success: true,
      data: list,
    };
    res.json(response);
...
router.get('/games/:id', (req, res) => {
  try {
    ...
    const response: ApiResponse<GameState> = { success: true, data: game };
    res.json(response);
  } catch (error) {
    const response: ApiResponse = {
      success: false,
...
router.post('/games/:id/reset', (req, res) => {
  try {
    ...
    const resetGame: GameState = { ...newGame, id: game.id, createdAt: game.createdAt };
    saveGame(resetGame);

    const response: ApiResponse<GameState> = { success: true, data: resetGame };
    res.json(response);
  } catch (error) {
    const response: ApiResponse = {
      success: false,
